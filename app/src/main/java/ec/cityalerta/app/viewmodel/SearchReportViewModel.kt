...
class SearchReportViewModel(
    ...
    // Realiza la busqueda con los filtros actuales via RPC search_reportes
    private fun search(query: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                val results = reporteRepository.searchReportes(
                    ciudadId = ciudadId,
                    categoria = _state.value.selectedCategory,
                    barrioNombreQuery = query.takeIf { it.isNotBlank() }
                ).getOrNull().orEmpty()

                val reportesUI = mapSearchResultsToUi(results)
    ...
    private suspend fun mapSearchResultsToUi(results: List<ReporteSearchResult>): List<ReporteUI> {
        if (results.isEmpty()) return emptyList()

        val reporteIds = results.map { it.reporte.id }
        val imagenes = reporteImagenRepository
            .getFirstImagenesByReporteIds(reporteIds)
            .getOrNull()
            .orEmpty()
        ...
        val signedUrls = reporteStorageRepository
            .generateSignedImageUrls(storagePaths)
            .getOrNull()
            .orEmpty()

        return results.map { result ->
            val imagen = imagenes[result.reporte.id]
            val imageUrl = imagen?.storage_uuid?.takeIf { it.isNotBlank() }?.let { signedUrls[it] }

            ReporteUI(
                id = result.reporte.id,
                categoria = mapCategoriaLabel(result.reporte.categoria),
                imageUrl = imageUrl,
                barrio = result.barrioNombre.ifBlank { "Barrio desconocido" },
                direccion = result.direccionAproximada ?: "Direccion no disponible",
                descripcion = result.reporte.descripcion,
                estado = mapEstadoLabel(result.reporte.estado),
                fecha = result.reporte.fecha_reporte,
                timeAgo = calculateTimeAgo(result.reporte.created_at)
            )
        }
    }
